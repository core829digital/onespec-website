...
import { useScroll, useTransform, useMotionValue, useReducedMotion } from "framer-motion";
...
// Utenti con "riduci movimento" attivo: nessuno scroll-jack, sezione statica a riposo.
function HeroStatic() {
...
function HeroScrollDriven() {
  ...
  // L'animazione delle linee deve concludersi ben prima che la sezione lasci lo
  // schermo: completa entro il 70% dello scroll disponibile, il restante 30%
  // resta come "cuscinetto" fermo prima del passaggio alla sezione successiva.
  const COMPLETE_AT = 0.7;
  const pathLengthFirst = useTransform(scrollYProgress, [0, COMPLETE_AT], [0.2, 1]);
  const pathLengthSecond = useTransform(scrollYProgress, [0, COMPLETE_AT], [0.15, 1]);
  const pathLengthThird = useTransform(scrollYProgress, [0, COMPLETE_AT], [0.1, 1]);
  const pathLengthFourth = useTransform(scrollYProgress, [0, COMPLETE_AT], [0.05, 1]);
  const pathLengthFifth = useTransform(scrollYProgress, [0, COMPLETE_AT], [0, 1]);
