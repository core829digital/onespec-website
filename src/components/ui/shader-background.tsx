...
// ShaderBackground — canvas WebGL "Silk" a tinta unica sfumata, palette
// onespec (nero + mint). Zero dipendenze, si riempie del genitore.
...
/**
 * Palette onespec: fondo quasi-nero -> mint scuro -> mint acceso, con coda
 * ripetuta sul mint per riempire l'array (colorCount conta solo le prime 4).
 * RGB in 0..1 (sRGB), convertiti dagli hex del brand:
 * #0A0B0D, #141618, #0FBF8F, #16D19D.
 */
const UNIFORMS = {
  colors: [
    [0.0392, 0.0431, 0.0510],
    [0.0784, 0.0863, 0.0941],
    [0.0588, 0.749, 0.5608],
    [0.0863, 0.8196, 0.6157],
    [0.0863, 0.8196, 0.6157],
    [0.0863, 0.8196, 0.6157],
    [0.0863, 0.8196, 0.6157],
    [0.0863, 0.8196, 0.6157],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    let inView = true;
    let disposed = false;
    // IntersectionObserver/visibilitychange possono flippare inView/visible a
    // false prima ancora del primo frame (es. tab aperto in background, o il
    // canvas non ancora "composito" quando l'observer scatta la prima volta).
    // Senza questa guardia il canvas resterebbe alla dimensione di default
    // del browser (300x150) e completamente trasparente finche non arriva un
    ...
    function requestRender() {
      ...
      if (!firstPaintDone || (visible && inView)) {
